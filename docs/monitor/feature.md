...
3. **步骤3：设置告警条件**
   - **阈值设定**：基于检测算法对指标数值设置条件（如严重级“≥90”、错误级“≥85”、警告级“≥80”）；基于算法本身也可对获取不到数据的情况（如`threshold/no_data`）触发条件；
...
- **灵活的接入口径（自动与手动）**：部分内置资源（如操作系统主机基础采集）支持全自动接入免配置，同时针对复杂对象提供**向导式的模板手动接入**流程。点击模板的「+接入」按钮，进入配置页完成3类核心配置：
